...
function getTestInfo(testType: string) {
  const testInfoMap: Record<string, {
    ...
    schulte: {
      title: '舒尔特方格测试',
      description: '舒尔特方格是一种经典的注意力训练工具，由德国心理学家舒尔特发明。它被广泛用于训练注意力分配能力、视觉搜索速度和眼球运动灵活性，对提升阅读速度和专注力有显著效果。',
      howItWorks: '在一个方格中随机分布着数字（如5×5方格中的1-25），您需要按照从小到大的顺序依次找到并点击这些数字。测试记录您完成整个方格所需的时间，时间越短表示注意力和视觉搜索能力越强。',
      factors: '舒尔特方格表现受注意力集中度、眼球运动灵活性、视觉搜索策略和练习程度影响。疲劳、分心和视力问题可能影响表现。定期练习可以显著提升注意力分配能力。',
      tips: [
        { title: '眼睛居中', content: '尽量让眼睛保持在方格中心位置，用周边视觉搜索数字，减少眼球移动距离。' },
        { title: '分区搜索', content: '可以将方格在心中分成几个区域，有系统地搜索每个区域。' },
        { title: '预判下一个', content: '在点击当前数字时，眼睛已经开始寻找下一个数字的位置。' },
        { title: '每天练习', content: '每天练习几分钟，持续练习比一次长时间训练更有效。' }
      ],
      benchmarks: {
        excellent: { range: '3×3: <5秒 | 4×4: <15秒 | 5×5: <25秒', description: '卓越的注意力和视觉搜索能力' },
        good: { range: '3×3: <8秒 | 4×4: <22秒 | 5×5: <35秒', description: '良好的注意力分配能力' },
        average: { range: '3×3: <12秒 | 4×4: <30秒 | 5×5: <45秒', description: '标准的视觉搜索水平' }
      }
    }
  };
